...
export default function Dashboard() {
  ...
  const [reassignTasks, { isLoading: isReassigning }] =
    useReassignTasksMutation();
  ...
  const overloadedMembers = teamSummary.reduce(
    (sum, { members }) => sum + members.filter((m) => m.isOverloaded).length,
    0
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <p className="text-sm text-gray-500 mb-4">
            Member workload and capacity overview
          </p>
          ...
          <div className="space-y-4">
            ...
            {teamSummary.map(({ team, members }) => (
              <div
                key={team.id}
                className="border rounded-lg border-gray-100 p-4"
              >
        ...
        >
          ...
          <p className="text-sm text-gray-500 mb-4">
            Latest task activities and reassignments
          </p>
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto">
              {recentActivity.map((log) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`${bgColor} rounded-full p-2 flex-shrink-0`}
                    >
          ...
          onConfirm={async () => {
            try {
              ...
              if (result.success) {
                ...
                if (count > 0) {
                  showToast(
                    `${count} task(s) reassigned successfully`,
                    "success"
                  );
                } else {
              ...
              showToast(
                error?.data?.error ||
                  "Failed to reassign tasks. Please try again.",
                "error"
              );
            }
          }}
